...
| `adr/0029` | Vite+ (`vp`) owns the check layer — `vp check` (Oxlint + Oxfmt) as the blocking `lint` gate, one-time Oxfmt baseline; builds stay Vite, tests stay vitest under bun, API stays alchemy/bun (ADR-0027/0028); template-owned files excluded via `template-sync.json`-derived ignores; web dev/build + root test to `vp` as approved follow-up                                                                                                                                                                                               |
