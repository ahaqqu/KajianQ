...
| `adr/0029` | Vite+ (`vp`) owns the check layer — `vp check` (Oxlint + Oxfmt) as the blocking `lint` gate, one-time Oxfmt baseline; API stays alchemy/bun (ADR-0027/0028); template-owned files excluded via `template-sync.json`-derived ignores; amended 2026-09-06: web dev/build/preview and root test run via `vp` (vitest suite verified under Node) — bun stays package manager, bun+alchemy stays API runtime/deploy; amended 2026-09-07: decision 5's version-coupling guard extended to the root `vitest`/`@vitest/coverage-v8` pins vs vp's bundled Vitest                                                                                                                                                  |
