...
| `adr/0028` | Alchemy (v2, Effect-native IaC) owns Cloudflare provisioning + deploys via `apps/api/alchemy.run.ts` (Worker/R2/DO/assets/secrets; pinned physical names, one-time `--adopt` bootstrap, `Cloudflare.state()` state store); Neon stays external; `wrangler dev` remains the dev/e2e runner; deploy.yml's dead D1 steps removed |
