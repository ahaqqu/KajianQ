...
- **Quran (implemented, issue #6):** `bun run ingest:quran` — acquire Tanzil Uthmani Arabic + Kemenag Indonesian translation + Quranic Arabic Corpus morphology (raw bytes archived to R2 through the ObjectStore seam) → integrity check (6,236 ayah / 114 surah / morphology coverage) → surah parents with cheap-tier LLM summaries (parent embedding computed from the summary, not full text) + per-ayah children (dual-track `embedding_ar` primary / `embedding_id` fallback per ADR-0013; lemma+root stored per Arabic token per ADR-0014) → aligned (AR, ID) ayah pairs persisted for #24's concept-graph build → IngestionReport persisted to `eval_runs`. Idempotent upserts; wiring goes through the generic `runIngestion` runner (ADR-0021).
