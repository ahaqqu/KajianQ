...
| `adr/0023` | Role-agent models resolve from `.zcode/agents/` pins on every harness — DSH reads the pin at dispatch (workflow mapping; unroutable ids carry a recorded fallback `glm-5.3`→`glm-5.2`) |
