...
- **Trace-anchored feedback.** Thumbs up/down per answer, plus flagging a specific Trace element: _wrong citation · irrelevant chunk · bad machine translation · questionable grade_. Free-text optional. Anonymous (template's anonymous sessions), rate-limited. Feedback lands in the admin review queue; accepted items become **Golden Set** cases.
...
| Cheap tier (router stages 1–3, tagging, cleaning) | Gemini 3 Flash Preview (live API id)                                      | DeepSeek V4-Flash ($0.14/$0.28) | $0.50/$3                |
| Generator                                         | **Qwen3.7-Max** (1M ctx, 119 langs)                                       | Gemini 3.1 Pro, Kimi K3         | $2.50/$7.50             |
| Reviewer (cross-vendor)                           | Gemini 3.1 Pro Preview (golden set), Gemini 3 Flash Preview (live sample) | Kimi K2.6                       | $2/$12, $0.50/$3        |
| Ingestion translation (AR→ID)                     | Qwen3 Max                                                                 | Gemini 3 Flash                  | $0.78/$3.90             |

**The embedding default is gate-decided (ADR-0036, 2026-09-10).** Per ADR-0013 (accepted), retrieval is Arabic-canonical: `text_ar`/`embedding_ar` is the primary evidence index; `text_id`/`embedding_id` is a built-from-the-start fallback/fusion track. The #9 benchmark gate compared `gemini-embedding-001` and `gemini-embedding-2` on ID→AR and AR→AR recall over the real corpus (self-retrieval probes, cosine recall@10) plus the ADR-0014 expansion micro-task; the numbers and the decided retrieval posture are recorded in `adr/0036-embedding-benchmark-gate-retrieval-posture.md` and the run report (`packages/kajianq-domain/fixtures/embed-bench-results.json`). The dual-index schema keeps the posture switchable without re-embedding. Gemini free-tier traffic is used for training by Google — acceptable for public religious content and anonymous queries; never route feedback free-text containing personal data through free tiers. Free-tier embedding quota counts _items_ (≈1,000/day per model) — sized into the gate's stratified-subset corpus.
