...
- **Privacy & GDPR by design** — any change touching personal data (session tokens, chat content, traces, feedback free-text, IPs/logs) keeps the sub-processor register, retention values, and RoPA (`docs/GDPR-ARTICLE-30-RECORD.md`) true in the same PR; a new processor or vendor joins the register in the same PR; personal data routes only through paid, DPA-covered vendors (`personalDataAllowed`, never a free tier); erasure stays complete (`DELETE /v1/auth/me` cascades the user's whole subtree); when the posture a PR changes is user-visible, the `/about` privacy notice must tell the same story — its copy is owned by the About-page privacy notice ticket (#179), so a posture change that outdates it files or pings that ticket in the same PR. Decision record: ADR-0043.
...
- [ ] Corpus/ingest changes: a verified `pre-ingest` snapshot exists before any paid run and a `post-ingest` snapshot after it; both labels and their manifests are cited in the PR (`bun run db:snapshot verify <label>`, ADR-0038).
