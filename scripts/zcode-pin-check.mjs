...
// not configured" (reproduced live 2026-08-30 — see ADR-0023's known-gap note)
...
if (failures > 0) {
  console.error(
    `✗ ${failures} of ${checks.length} pin(s) cannot resolve on ZCode — fix the provider config, never reroute the pin (ADR-0023). Note: a config fixed mid-session needs a client restart before spawns honor it.`,
  );
  process.exit(1);
}
