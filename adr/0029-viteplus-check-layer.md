...
4. **Scope boundary: the check layer only.** `apps/web` keeps `vite dev`/`vite build`, tests stay on vitest under the bun runtime, package management stays bun, and `apps/api` stays `alchemy dev`/`alchemy deploy` (ADR-0028) on bun (ADR-0027). The approved follow-up — renaming web `dev`/`build` and root `test` to their `vp` equivalents after verifying vitest-under-Node compatibility — is a mechanical, low-risk PR that does not need a new ADR; anything beyond that (vp task runner, vp pack) does.
