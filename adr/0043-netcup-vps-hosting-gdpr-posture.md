...
3. **Sub-processor register** (this table is the source the `/about` privacy
   ...
   | Cloudflare, Inc. (US)      | **Post-cutover (2026-09-21): at-rest archives only.** R2 (`kajianq-raw-staging`) holds the raw-corpus exports and the snapshot dumps (provenance archive, ADR-0038 decision 4). The Workers runtime, static assets, and Durable Objects were deleted at cutover; no serving traffic transits Cloudflare | snapshot/backup archives and corpus raw exports at rest (no serving-path personal data)                                                                 | free tier (storage)                                                      | Permitted for at-rest archives only; no runtime personal-data processing remains                                          |
   | Neon, Inc. (US)            | **Decommissioned 2026-09-21** — the project was deleted after the post-cutover archive verified in two places; the row is kept for the register's history and still carries a tier and a verdict                                                                                                        | — (no processing remains)                                                                                                                               | free plan (historical)                                                   | Closed; the whole DB now lives on the netcup VPS                                                                          |
   | Google (Gemini API)        | LLM: fallback candidate on the `cheap` role (DeepSeek heads the chain) and embeddings for **non-personal calls only** — **not** the reviewer since the 2026-09-21 amendment re-headed it to DeepSeek                                                                                                    | prompt/embedding content of any call flagged personal (in practice: corpus ingestion and tagging, both non-personal)                                    | free tier — free-tier traffic may be used to improve the vendor's models | **NOT permitted** for personal-data calls (`personalDataAllowed: false`, already encoded)                                 |
   | Google (Gemini API, paid)  | LLM: the `embedder` role on **paid/standard terms** — the same API and the same model id as the row above, keyed separately because the register rule keys on terms, not on the model                                                                                                                   | the user's sub-queries embedded by the serving retriever (personal data)                                                                                | paid                                                                     | Permitted; the same embedding space as the free-tier row, so the already-ingested corpus vectors stay valid (no re-embed) |
