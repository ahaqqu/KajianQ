...
3. **A pin that fails to resolve on a harness is a harness-config gap, fixed in the harness's own configuration — never rerouted to a different model.** On DSH, declare the model id in the provider's model list (`~/.dsh/settings.yaml`, hot-reloaded) and re-probe. Per-harness fallback models are rejected: they fork the pin truth, make the DSH run silently weaker than the pin declares, and bury the deviation inside skills. The role files keep their template pins untouched (template-sync conflicts avoided). Skill and adapter files record no fallback tables — the *absence* of routing deviations is the invariant; probe evidence and dates live in this ADR and the PR, not in skills.
