...
2. **Every harness honors the pin.** ZCode does so natively (override order: user → project pin → template default). DSH does so through its adapter (`.agents/skills/manager/DSH-ADAPTER.md`): at dispatch, read the pin, translate `ollama/<model>:cloud` → `{ provider: "ollama", model: "<model>" }`, resolve the effective model against the DSH routing table there, then dispatch — plain `subagent` (continuable) when the effective model equals the session model, otherwise a `workflow` `agent()` call (one-shot; a CI-red relay respawns a fresh agent carrying the failing logs instead of resuming).
