 ...
 * the canonical-evidence track and `textSecondary` the display track
 * (ADR-0013 roles), and `morphology` is the verbatim per-token annotation.
 */
export type AlignedPairInsert = {
  /** Caller-supplied provenance key — the store upserts on it (rule 11). */
  pairKey: string;
  citation: Record<string, unknown>;
  textPrimary: string;
  textSecondary: string;
  /** Verbatim `MorphToken`-shaped annotation from the morphology source. */
...
export interface RagStore extends RagStoreEvalRunWrite, RagStoreEvalLedger {
  ...
  // -- Feedback (#13, ADR-0007) ---------------------------------------------

  /**
   * Persist one feedback row (anonymous thumb or trace-anchored flag) against
   * an answer message. The anchor vocabulary is the CALLER's: `anchorType`,
   * `anchorId`, and `category` are opaque pass-through values, exactly like
   * `metadata` — the engine stores them verbatim and interprets nothing. The
   * `feedback.rating` column admits only −1/1; the shared contract decides
   * which wire shape maps to which.
   */
  ...
  /**
   * The feedback target for one answer message (#13): the persisted Trace plus
   * its owning user id, or null when no trace exists for the message. The
   * caller validates the anchor against the trace and the ownership against
   * the authenticated user — the store just fetches.
   */
