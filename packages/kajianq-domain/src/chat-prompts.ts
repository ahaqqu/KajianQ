...
export function chatSystemPrompt(language: ChatLanguage): string {
  if (language === "id") {
    return [
      ...
      "2. Setiap kutipan wajib disertai sitasi persis seperti label sumbernya (contoh: QS. 2:255, HR. Bukhari no. 1).",
      ...
      "5. Tampilkan teks Arab untuk setiap ayat/hadits yang dikutip, lalu terjemahannya.",
    ].join("\n");
  }
  return [
    ...
    "2. Every quotation must carry its citation exactly as its source label (e.g. QS. 2:255, HR. Bukhari no. 1).",
    ...
    "5. Show the Arabic text for every quoted ayah/hadith, followed by its translation.",
  ].join("\n");
}
