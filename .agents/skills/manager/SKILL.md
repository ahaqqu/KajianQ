...
The fixer reuses A's worktree (`/tmp/wt-<branch>`) if it still exists, or adds a fresh one from the existing branch (`git worktree add /tmp/wt-<branch> <branch>` — no `-b`). State the worktree path in C's dispatch prompt.
