...
A owns CI green: it watches its own checks, iterates on red, and reports done only when the PR exists and all checks pass. You do not monitor CI between dispatches. The one continuous monitoring duty you carry while a subagent runs is the efficiency watchdog — cost/telemetry signals only, never CI.
