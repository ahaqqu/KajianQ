...
- **Model routing:** resolved by ZCode from the pin files' `model:` frontmatter — override order: `~/.zcode/agents/<role>.md` (user) → this project's `.zcode/agents/` → template pin (see the pinned-defaults table in the role registry). Nothing to map at dispatch time; a model-pinned dispatch keeps `SendMessage` continuation, unlike DSH.
