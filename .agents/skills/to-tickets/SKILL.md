...
The project issue tracker is configured per-project. Publish tickets to the configured tracker (defaults to `.scratch/<feature-slug>/issues/` as local files if no tracker is set up). Apply the `ready-for-agent` triage label.
...
Publish the approved tickets. **How** depends on the project's issue tracker — the tickets are the same either way, only the shape of the blocking edges changes:
