...
- **Thermos (mandatory for code)** — if the diff touches any runtime code (`apps/`, `packages/`, `scripts/`, migrations, CI workflows), run `.agents/skills/thermos-with-comments/SKILL.md`: run both thermo passes yourself, synthesize, and post the itemized findings as PR comments. This is not optional and not a recommendation — a PR that changes code is always reviewed at thermos depth.
...
All findings — the philosophy/guardrail violations from this pass and the thermos findings — are posted per the posting contract in `.agents/skills/thermos-with-comments/SKILL.md`: one individual review comment per finding with a stable ID, line-anchored inline comments by default with a justified PR-level fallback, and the stale pending-draft preflight before posting. A summary comment may index the items but must never be the only place a finding exists.
