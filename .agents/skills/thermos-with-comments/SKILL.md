...
2. Run the **security/correctness** pass yourself using the standards in `.agents/skills/thermo-nuclear-review/SKILL.md`. Produce a prioritized, numbered list of findings (`A1, A2, …`).
3. Run the **maintainability** pass yourself using the standards in `.agents/skills/thermo-nuclear-code-quality-review/SKILL.md`. Produce a prioritized, numbered list of findings (`B1, B2, …`).
4. Synthesize: merge the two passes. Deduplicate overlapping findings, weight overlaps more heavily, resolve disagreements with your own judgment. Do not restate each pass wholesale — the deliverable is one unified, itemized report.
5. Assign each finding a **stable item ID**:
   - `A1, A2, …` — security / correctness findings.
   - `B1, B2, …` — code-quality / maintainability findings.
   - `C1, C2, …` — post-synthesis / cross-cutting findings (conflicts, systemic issues, or findings neither pass caught alone).
   - Never renumber after publication. If a finding is withdrawn later, mark it `withdrawn — <reason>`; do not reuse its ID.
6. Build each item's payload. Every item has:
...
9. Post one **summary comment** (via `gh api repos/{owner}/{repo}/issues/{n}/comments`) with:
...
10. Return to the caller (the manager or `code-review`): the PR URL, the full itemized report (all IDs, priorities, files, one-line summaries, and each item's posted review-comment ID so dispositions can thread on the original comment), and the posted-comment verification result.
