...
- **Per-harness deviations are configuration, recorded in one place.** A dated routing/fallback table in that harness's adapter file beats both silent drift and edits to the shared files — template-synced pins stay untouched, and the deviation stays auditable where the dispatch rule reads it.
