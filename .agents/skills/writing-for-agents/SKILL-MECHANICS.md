...
- **Cut on the adapter boundary, and only there.** Per-harness mechanics (spawn/resume/model/isolation) go into a per-harness **adapter file**, reached by a router pointer in the shared body that the agent resolves from its own spawn tools (see the `manager` skill's _Harness adapters_ router → `ZCODE-ADAPTER.md` / `DSH-ADAPTER.md`) — so a session loads only its own branch. The loop itself, the shared role config, and workflow steps stay single-sourced: splitting behavior forks the protocol and every future change gets made twice. A harness-specific mechanic stated in the shared body is a variance bug: runs on the other harness take a path the text does not describe.
