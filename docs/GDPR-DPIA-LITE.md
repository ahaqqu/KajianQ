...
| **Deliberate log retention**                  | Access logs are 14 days, so the IP-bearing log cannot become the de-facto record of who asked what; structured API logs carry a correlation id and no IP                                                               | ADR-0043 decision 4; implemented as code in `provision/vps/logrotate/kajianq-proxy` + `provision/vps/nginx/kajianq.conf` (#180)                                                                |
...
| **Encrypted backups with re-applied erasure** | Backups are encrypted at rest with an off-repo key, 30-day rolling, and never used to answer a subject request; after a restore the reclamation/erasure path is re-run so a restored row cannot resurrect deleted data | ADR-0043 decision 4; `provision/vps/backup/` (#180) — the restore drill proves the resurrection case is caught and corrected, run by `.github/workflows/vps-restore-drill.yml`                 |
...
| Transitional free-tier rows (Cloudflare, Neon) process personal data while the migration is pending | **Closed (2026-09-21).** The migration executed: the Workers runtime, static assets, and Durable Objects are deleted, the Neon project is deleted, and no serving traffic transits either. Both rows stay registered as `no-serving-role` for the register's history (R2 remains an at-rest provenance archive) — see `docs/VPS-CUTOVER-RECORD.md` step 7.                                                                                                                                                                                          |
