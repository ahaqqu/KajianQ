...
| 2026-09-26 (VPS, full corpus, #213) | bukhari, muslim, abudawud, tirmidhi, nasai, ibnmajah (one pass each, `--offset N --limit 1`) | `pre-ingest-20260926t0642z` (127.8 MB) + per-pass `pre-ingest-20260926t{0722z,0802z,0837z,0908z,0952z}`; `post-ingest-20260926t1112z` (637.6 MB) — all verify-clean | bukhari 7,580 · muslim 7,360 · abudawud 5,272 · tirmidhi 3,924 · nasai 5,679 · ibnmajah 4,338 — each equals its declared `--check` count; quran 6,236 + malik 1,829 untouched (`created_at` still 2026-09-12). Store now 42,511 children / 804 parents / 37,014 pairs | 2,838 micro-USD total (709+571+409+416+417+316 per pass); every pass `archiveStored: true` with its report in `eval_runs` (runIds `5fc20306`, `aa6b5a91`, `590a35c0`, `ecf00c88`, `0492a146`, `2ada1d3f`) | DB 299 MB → 1,431 MB; `/srv` 2.5 GB → 4.2 GB of 125 GB. Snapshot `create` over the tunnel outgrew a 580 s window once the dump passed ~500 MB (one attempt killed; labels immutable, re-run clean) — run it without a timeout. Post-run health: the `Staging` workflow's Golden Set smoke against the widened store — 5/5 passed, 0 skipped, **mean retrieval recall 1.000**, citation validity 1.000 (Staging run `36217497055`, eval run `aee062dc`; ZAP + Schemathesis green on the same run) |
