...
| AC-3  | Ingest/eval CLIs run against the VPS database                                              | The Golden Set smoke green against the VPS store; `--check` passes reported "store untouched, no LLM/embedding spend"; `db:status:all` fully applied. **Post-record confirmation (2026-09-25):** the Staging workflow opens an ssh tunnel to the VPS database and runs `eval:smoke` against it through `STAGING_DATABASE_URL` (tunnel port 15433) — green at 5/5, 0 skipped, `mean retrieval recall: 1.000`; and the vendor-free `DATABASE_URL` is what every CLI reads (no `NEON_DATABASE_URL` remains in `packages/` or `apps/`)                | Owner + Repo                |
| AC-4  | API serves `/v1/*` with TLS; the anonymous-session cron runs on the host                   | "Live-flow verification": unit active, timer scheduled at 03:17, HTTPS health JSON, `journalctl -u kajianq-cron` showing a completed run. **Post-record confirmation (2026-09-25):** every green deploy re-runs the reclamation once (`deploy: running the reclamation once (proves the cron entry executes)`) and smoke-checks `/v1/health`, `/v1/auth/anonymous`, and `/chat` over TLS                                                                                                                                                          | Owner                       |
| AC-5  | Deployer separation                                                                        | `grep -rn "alchemy" apps/` returns nothing tracked (`alchemy.run.ts`, `wrangler.toml`, and the `alchemy:*`/`deploy:bootstrap*` scripts are all deleted); `deploy-vps.yml` and `provision/vps/deploy/` exist                                                                                                                                                                                                                                                                                                                                       | Repo                        |
...
| AC-8  | Smoke tests pass against the VPS; e2e re-pointed                                           | 33/33 BDD scenarios green locally; the deploy script's smoke lines against the public URL. **Post-record confirmation (2026-09-25):** `playwright.config.ts` boots the same Bun entry the VPS serves (`bun apps/api/src/boot.ts`, ADR-0044) instead of `alchemy dev`, and the deploy's full post-checks suite is green (ZAP `FAIL-NEW: 0 / WARN-NEW: 0 / PASS: 62`; Schemathesis 1958 passed with 2 documented warnings; Golden Set smoke 5/5)                                                                                                    | Repo + Owner                |
...
| AC-11 | About-page register flips at cutover                                                       | The drift guard green with netcup `current` and the transition rows narrowed. **Corrected at close-out (2026-09-25):** the guard exposed a row the register was missing entirely — `gemini-paid`, the paid-terms embedding row that carries the user's sub-queries (personal data). The free-tier Google row's "non-personal calls only" verdict was doing the disclosure work for both; the paid row is now registered in ADR-0043, the notice, and the Art. 30 record, and `CATALOG_IDS` maps it to `models.json` so the two cannot drift again | Repo                        |
| AC-12 | Backup timer + cron re-homed                                                               | Both timers scheduled; `kajianq-cron` proven by the deploy's reclamation run. **The backup half was broken twice after this record and is now fixed and proven by an operator-started run** — see "the nightly backup had never worked" and "the two follow-on defects": the scheduled 03:15 runs of Sep 21–25 all failed on the placeholder, and the first success is `daily-20260925t121812z` at 14:18 CEST (133,598,276-byte dump, `ExecMainStatus=0`). A **timer-triggered** run is still outstanding — #214                                  | Owner + #214                |
| AC-13 | Decommissioning Cloudflare + Neon + CF secrets                                             | Step 7's deletion confirmations; `gh secret list` without the CF/Neon entries; the grep returning nothing. **Verified at close-out (2026-09-25):** repo secrets are exactly `DEEPSEEK_API_KEY`, `GEMINI_API_KEY`, `RATE_BYPASS_PRIVATE_KEY`, `STAGING_DATABASE_URL`, `VPS_DEPLOY_SSH_KEY` — no `CLOUDFLARE_*`, no `NEON_*`; repo vars are `PROD_URL`, `VPS_HOST`, `VPS_PUBLIC_URL`, `VPS_USER`                                                                                                                                                    | **Owner approval required** |
